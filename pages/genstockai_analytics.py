...
if inventory:
    ...
    for product, data in inventory.items():
        # Handle both old format (direct int) and new format (dict with 'quantity' key)
        if isinstance(data, dict):
            qty = data.get('quantity', 0)
            last_scanned = data.get('last_scanned', 'N/A')
        else:
            qty = data
            last_scanned = 'N/A'
        
        # Find velocity for this product
        velocity = 0
        for p in products:
    ...
    # Sort by weeks of supply (lowest first)
        ...
        with st.container(border=True):
            ...
            with col1:
                ...
                if item['last_scanned'] != 'N/A':
                    st.caption(f"Last scanned: {item['last_scanned'][:10]}")
